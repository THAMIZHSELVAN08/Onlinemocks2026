...
export default function HRDashboard() {
  ...
  const pending = students.filter(s => s.status === 'PENDING' || s.status === 'IN_PROGRESS').length;
  const total = students.length;

  const chartData = [
    { name: "Verified", value: completed, color: "#10b981" },
    { name: "Awaiting", value: pending, color: "#334155" },
    { name: "Total", value: total, color: "#CBD5E1" },
  ];
  ...
  return (
    <div className="flex bg-[#F7F8FA] min-h-screen font-sans text-slate-900 selection:bg-blue-600/10 selection:text-blue-600 relative">
      {/* ── Sidebar ── */}
      <aside className={`bg-white border-r border-slate-100 flex flex-col fixed inset-y-0 z-50 p-6 overflow-y-auto custom-scrollbar transition-all duration-300 ${isSidebarCollapsed ? 'w-24' : 'w-64'}`}>
        <div className={`flex items-center gap-3 px-2 mb-10 transition-all ${isSidebarCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isSidebarCollapsed && <span className="font-bold text-slate-900 tracking-tight">HR Portal</span>}
        </div>
        ...
        <nav className="space-y-1">
          ...
          <div className={`mt-8 mb-3 px-3 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] leading-none ${isSidebarCollapsed ? 'text-center opacity-0' : ''}`}>Management</div>
          <SidebarLink active={activeTab === "students"} onClick={() => setActiveTab("students")} icon={Users} label={isSidebarCollapsed ? "" : "Students List"} />
          <SidebarLink active={activeTab === "feedback"} onClick={() => setActiveTab("feedback")} icon={Star} label={isSidebarCollapsed ? "" : "Event Feedback"} />
      ...
      >
        <ChevronLeft size={16} className={`text-slate-400 group-hover:text-blue-600 transition-all ${isSidebarCollapsed ? 'rotate-180' : ''}`} />
      </button>
